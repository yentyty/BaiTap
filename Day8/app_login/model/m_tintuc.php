<?php 
	...
	class tintucDB{
		...
		public static function get_tintucmoi($batdau, $soluong){
			$db = Database::getDB();
			$query = "SELECT * FROM tintuc ORDER BY ngaytao DESC LIMIT $batdau, $soluong";
			$statement = $db -> prepare($query);
			$statement -> bindValue(':soluong',$soluong);
			$statement -> execute();
			$tinmoi = $statement->fetchAll();
			$statement -> closeCursor();
			$result = array();
		...
		public static function get_tinby_idloaitin($idloaitin){
			$db = Database::getDB();
			$query = "SELECT * FROM tintuc
						WHERE idloaitin = ?" ;
			$statement = $db -> prepare($query);
